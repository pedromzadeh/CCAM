...
class Force():
    ...
    def cahn_hilliard_func_deriv(self, cell):
        ...
        """
        _, _, lap = utils.compute_gradients(cell.phi, cell.simbox.dx) 
    ...
    def area_func_deriv(self, cell):
        """
        Computes the functional derivative of the area conservation term w.r.t field phi.
    ...
    def substrate_int_func_deriv(self, cell):
        """
        Computes the functional derivative of the cell-substrate interaction energy w.r.t field phi.
    ...
    def cell_adhesion_func_deriv(self, cell, cells):
        """
        Computes the functional derivative of the cell-cell adhesion energy w.r.t field phi.
        ...
        return -4 * self.omega/cell.lam * cell.phi * (2*cell.phi - 1) * (cell.phi - 1) * neighbors_present
    ...
    def actin_motility_force(self, cell, chi, grad_phi, x_hat):
        """
        Computes the motility force as $f = \beta * (p * x) * |grad phi| |grad chi| * H(p * n) * x$. This approximates forces generated at the cell front by actin polymerization, and it leads to lamellipodia. 
        ...
        grad_chix, grad_chiy, _ = utils.compute_gradients(chi, cell.simbox.dx)
        grad_chi = np.array([grad_chix, grad_chiy])
        p_field = np.array(
            [np.ones(phi_i.shape)*p_i[0], 
            np.ones(phi_i.shape)*p_i[1]]
            )
